  ...
  function(session, results) {
    if (results.response) {
      ...
      session.replaceDialog("/", {
        promptMessage: session.dialogData.promptMessage
      });
    }
  }
]);
...
lib.dialog("billing", [
  function(session, args, next) {
    ...
    var saved = (session.userData.billingAddresses =
      session.userData.billingAddresses || {});
    if (hasAddresses(saved)) {
      ...
      if (selection && saved[selection]) {
        ...
        session.gettext(UseSavedInfoChoices.NotThisTime).toLowerCase()
      ) {
        // Ask for data
        next();
      } else {
        ...
        var message = new builder.Message(session).attachmentLayout(
  ...
  function(session, args, next) {
    ...
    if (
      args.response &&
      args.response.entity !== session.gettext(UseSavedInfoChoices.NotThisTime)
    ) {
      // Save address
      session.userData.billingAddresses =
        session.userData.billingAddresses || {};
...
function hasAddresses(addresses) {
  return (
    addresses[UseSavedInfoChoices.Home] || addresses[UseSavedInfoChoices.Work]
  );
}
...
function createAddressCard(session, buttonTitle, address) {
  return new builder.HeroCard(session)
    .title(buttonTitle)
    .subtitle(address)
    .buttons([builder.CardAction.imBack(session, buttonTitle, buttonTitle)]);
}
