...
lib.dialog("/", [
  function(session) {
    ...
    session.beginDialog("address:/", {
      promptMessage: session.gettext(
        "provide_delivery_address",
        session.message.user.name || session.gettext("default_user_name")
      )
    });
  },
  ...
  function(session, args) {
    ...
    session.send(
      "confirm_choice",
      session.dialogData.selection.name,
      session.dialogData.deliveryDate.toLocaleDateString()
    );
